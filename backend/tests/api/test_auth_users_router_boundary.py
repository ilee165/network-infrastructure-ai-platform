"""Architecture guard for the admin-users HTTP/service boundary."""
...
ROUTER = Path(__file__).parents[2] / "app" / "api" / "v1" / "auth" / "users.py"


def test_auth_users_router_is_orm_free() -> None:
...
def test_auth_users_router_preserves_boundary_types() -> None:
    """Dependency and projection annotations may not be erased to ``Any``."""
    tree = ast.parse(ROUTER.read_text(encoding="utf-8"))
    any_references = [
