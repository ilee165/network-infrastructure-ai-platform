...
class SpatiumCredentials:
    ...
    ``appliance_id`` is a non-secret, human-assigned identifier for the
    SpatiumDDI appliance (e.g. the hostname or a slug). It is included in
    ``__eq__`` and ``__hash__`` (both auto-generated by the frozen dataclass) so
    that two :class:`SpatiumCredentials` objects for *different* appliances are
    distinguishable when used as dict keys or set members — which the bearer
    ``token`` alone cannot guarantee because it is excluded from compare/hash to
    prevent accidental exposure via reflection.
    """
...
class SpatiumClient:
    ...
    # -- DNS (DDI_DNS + DISCOVERY_API) -------------------------------------

    async def get_dns_groups(self) -> list[dict[str, Any]]:
        """GET all DNS server groups (bare list).

        DISCOVERY_API fan-out root — enumerates the top-level DNS groups whose
        IDs are then passed to :meth:`get_zones` to enumerate child zones
        (ADR-0024 §1 DISCOVERY_API).
        """
        return await self._get_list("/dns/groups")
    ...
    # -- DHCP (DDI_DHCP + DISCOVERY_API) -----------------------------------

    async def get_dhcp_scopes(self, group_id: str) -> list[dict[str, Any]]:
        """GET the DHCP scopes of a server group (bare list).
    ...
    async def get_spaces(self) -> list[dict[str, Any]]:
        """GET all IPAM spaces (bare list).

        DISCOVERY_API fan-out root — enumerates the top-level address spaces
        (``GET /ipam/spaces``, ADR-0024 §1 DISCOVERY_API).
        """
        return await self._get_list("/ipam/spaces")

    async def get_blocks(self) -> list[dict[str, Any]]:
        """GET all IPAM blocks (bare list).

        DISCOVERY_API fan-out root — enumerates aggregate address blocks
        (``GET /ipam/blocks``, ADR-0024 §1 DISCOVERY_API). Blocks sit between
        spaces and subnets in the IPAM hierarchy.
        """
        return await self._get_list("/ipam/blocks")
