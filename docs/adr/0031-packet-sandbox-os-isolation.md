...
- **Container-runtime init syscalls (runc ≥1.2, added 2026-07-01):** the allow-list additionally permits the new mount-API family — `fsopen`, `fsconfig`, `fsmount`, `fspick`, `open_tree`, `move_mount`, `mount_setattr`, plus `statfs`/`fstatfs`. runc ≥1.2 uses these to *safely re-open* `/proc/self/fd` during container setup — its CVE-2024-21626 ("Leaky Vessels") mitigation. Without them the analysis container crash-loops at init with `reopen exec fifo: get safe /proc/thread-self/fd handle: fstatfs fsmount:fscontext:proc: operation not permitted`. These mirror moby's own default hardened profile and are **inert without `CAP_SYS_ADMIN`**, which this sandbox drops (`cap_drop: [ALL]` + no-new-privileges): the parser gains no mount/namespace capability, so `mount`/`unshare`/`setns` and the raw-socket primitives above **stay denied**. This is exactly the fail-closed upgrade gate noted in Consequences — a runtime (not just tshark/pyshark) bump that needs a new init syscall requires a reviewed profile bump.
...
> **Deferred (2026-07-01) — container-level socket-deny vs the Celery broker.** The analysis workload is a **Celery worker**: it must open a TCP socket to the Redis broker to receive tasks. But this profile denies `socket` at the *container* level, which also denies glibc's DNS resolver its UDP socket — so the worker cannot even resolve `redis` (`Temporary failure in name resolution`) and never connects. Because tshark is spawned by plain `subprocess.run` with **no subprocess-level seccomp**, one container-level profile cannot both permit the worker's broker socket and deny the parser's sockets. This also makes §3 (deny `socket`) and §4 (a default-deny egress NetworkPolicy that *allows* redis/postgres) internally inconsistent — the NetworkPolicy is only meaningful if sockets are permitted at all. Pending resolution, packet-analysis ships **opt-in / default-OFF** (Compose `--profile packet`; chart `packet.analysis.enabled=false`) so the core platform runs clean. Two candidate resolutions: **(a) nested confinement** — run the worker under `RuntimeDefault` and apply this deny-socket profile only to the tshark child via a seccomp-installing launcher (`PR_SET_NO_NEW_PRIVS` + `seccomp()` then `execve`); or **(b) a §3 amendment** — allow *client* socket syscalls (no `bind`/`listen`/`accept`) and make the §4 egress NetworkPolicy (egress only to redis/postgres) plus the dropped `CAP_NET_RAW` (no `AF_PACKET`) the network-containment boundary. **(a)** preserves the strongest posture for the untrusted parser and is the preferred target. The other OS-isolation controls (non-root, `cap_drop:[ALL]`, read-only rootfs, resource bounds, dedicated node pool) are unaffected and remain in force when the workload is enabled.
