...
3. **Live lease endpoint shape** — `GET /dhcp/servers/{server_id}/leases` returns live Kea/Windows leases whose population depends on a running DHCP agent; the exact freshness/empty behavior (and whether `sync-leases` must run first) is only observable live.
