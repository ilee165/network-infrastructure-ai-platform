...
**Status:** Planned (not built). Awaiting go to launch W0.
...
|---|---|---|
| `wf-implementer` | strong (inherit) | Novel/security-critical: OIDC, KMS, sandbox isolation, DR data-integrity path, NetworkPolicy/PSS semantics |
...
**Infra/DevOps gap:** no dedicated infra role. K8s/Helm/Trivy/SBOM work is YAML + CI, not Python-TDD. `wf-implementer` (all-tools, strong) covers security-semantic infra (NetworkPolicy, PSS, admission); `wf-implementer-light` covers boilerplate manifests. Adequate — no new agent. Watch: YAML review depth is weaker than code; assign a strong reviewer to NetworkPolicy/PSS/admission tasks.
...
| **W3 — Packet sandbox + K8s hardening R1** | Sandbox OS-isolation (non-root, dropped caps, NET_RAW node-pool, seccomp, default-deny egress); PSS `restricted` + readOnlyRootFS + resource limits + K8s RBAC least-priv | `wf-implementer` (strong) | **strong** quality | Clears M5 PARTIAL packet-sandbox sign-off |
| **W4 — Helm / K8s GA chart** | Per-service Deployments + NetworkPolicies (§3.1 topology; HA replicas excluded → P2); hardened-default values + cert-manager TLS ingress + admission policy | light (boilerplate) + strong (NetworkPolicy/admission) | strong | Secure-by-default = opt-out, never opt-in |
| **W5 — Backup / DR baseline** | pgBackRest (WAL + full/incr to MinIO); Neo4j rebuild-drill job (emits topology-RTO metric); pcap volume snapshot | `wf-implementer` (strong — audit-integrity path) | **strong** quality | Drills run from P2; RPO ≤ 5 min / RTO ≤ 1 h PROPOSED targets |
| **W6 — Security hardening (P1 subset)** | Master key → KMS via D11 interface + rotation/re-wrap; CI pip-audit + npm-audit + gitleaks + SBOM (syft) + cosign signing; Redis-backed rate-limit + login lockout | `wf-implementer` (strong) | **strong** spec + quality | All secret-surface → escalated |
