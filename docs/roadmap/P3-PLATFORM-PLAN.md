...
**W3 (SIEM + SLO):** the §6 SLIs are **backed by emitted `netops_*` series** (T0:
HTTP middleware + domain counters + served `/metrics`; cardinality-bounded, no
payload leak; reconciliation-only SLIs flagged not fabricated) — so the recording
rules below derive from real series, not absent ones; the SIEM export pipeline
emits valid RFC5424 syslog + CEF +
