...
| **W4 — evals + phase exit** | **In progress** | Start with W4-T0A/T0B, then T1 → T2A → T2 → T3. W4-T2 preflight after T1 identified the bounded T2A derivation-runtime correction required before the eval-only T2; T4 alone owns the ledger's final lifecycle/status and revalidation at one combined release HEAD. |
