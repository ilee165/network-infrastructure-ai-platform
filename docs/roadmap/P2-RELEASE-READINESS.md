...
| `kind-harness` (enforcing-CNI self-test + assertions — **non-blocking**) | static harness-invariant validator + assertion-library self-tests + `extract_secret.py` tests bite (blocking steps); the **live** `kind-harness.sh` enforcement run is `continue-on-error` and the job is **deliberately ABSENT from `all-gates` needs** (ci.yml:716, 779) | **SUCCESS** (blocking steps green; live run is signal-only — see §2 G-SEC mTLS/collector lines) |
...
| **mTLS api/worker↔postgres: cert material renders idempotently; handshake asserted + plaintext refused** (§5 P2, ADR-0039) | **PARTIAL — material/static layer PASS (blocking); live kind handshake DEFERRED** | The mTLS render-twice **L4 idempotency guard** (`ci/mtls/render-twice.sh`) and the external-PG fail-fast guard are **RED gates** in the `infra` job (no continue-on-error, ci.yml:650/660); `extract_secret.py` hardening tests bite in `kind-harness`. The **live api/worker↔pg handshake + plaintext-refused assertion on a kind cluster** runs in `kind-harness.sh` which is `continue-on-error` and **not in `all-gates`** — so a live-handshake regression does **not** currently bite. Recorded as deferred-accepted (no-hardware on the authoring host; promotion path in `docs/runbooks/kind-harness.md`). [W4-T4] |
