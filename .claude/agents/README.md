...
| `wf-observability`     | inherit (strong) | all        | SLI/SLO enforcement (G-OBS): Prometheus recording rules, multi-window burn-rate alerts, Grafana dashboards-as-code, fault-injection MTTD harness. Alert-as-test gates (`promtool check`/`test rules` with a firing BITE proof), not Python-TDD. Added P3-Platform |
| `wf-reliability`       | inherit (strong) | all        | G-SCA/G-REL live-drill harnesses: load gen (k6/locust), chaos pod-kill failover, RTO/RPO/idempotency + queue-burst + compressed-soak drills against an ephemeral HA kind cluster. Drill-as-test gates (drill runs AND bites; honest reduced-scale + named ceiling), not Python-TDD. Added P3-Platform |
